...
_UPDATE_BANNER_SCRIPT = r"""
<script>
/* Cronus auto-update banner - injected by ui_dashboard.py */
(function () {
  var TOKEN = (document.querySelector('meta[name="cronus-api-token"]') || {}).content || "";
  var box = null;
  ...
  function show(message, buttonLabel, onClick) {
    if (!box) {
      box = document.createElement("div");
      box.id = "cronus-update-banner";
      box.style.cssText =
        "position:fixed;left:50%;transform:translateX(-50%);bottom:18px;z-index:99999;" +
        "display:flex;align-items:center;gap:14px;max-width:min(92vw,680px);" +
        "background:#12131a;border:1px solid #34374a;border-radius:12px;color:#e3e5ee;" +
        "padding:10px 16px;box-shadow:0 10px 34px rgba(0,0,0,.55);" +
        "font:500 12px/1.45 'Kanit','Segoe UI',sans-serif;";
    ...
    if (buttonLabel) {
      ...
      btn.style.cssText =
        "flex:0 0 auto;background:#2563eb;color:#fff;border:0;border-radius:8px;" +
        "padding:7px 16px;cursor:pointer;font:600 12px 'Kanit','Segoe UI',sans-serif;";
  ...
  async function poll() {
    try {
      var r = await fetch("/api/update/status", { headers: headers(false) });
      var s = await r.json();
      if (!s || !s.enabled) { hide(); return; }
      var cur = shortCommit(s.build_commit_short);
      var next = shortCommit(s.release_commit);
      if (s.update_available || s.download_pending) {
        var msg = "Update " + (next || "") + " available" + (cur ? " (you are on " + cur + ")" : "") + ". ";
        if (s.download_pending) {
          msg += "It will apply when Cronus exits.";
          show(msg, "Update & restart", function () {
            fetch("/api/update/apply", { method: "POST", headers: headers(true), body: "{}" }).catch(function () {});
          });
        } else if (s.downloading) {
          show("Downloading update...", null, null);
        } else {
          show(msg, "Download & restart", function () {
            fetch("/api/update/apply", { method: "POST", headers: headers(true), body: "{}" }).catch(function () {});
          });
        }
      } else {
...
_EXPLOITSTRAP_PICKER_SCRIPT = r"""
<script src="/ui/runtime/exploitstrapVersionPicker.js?v=5"></script>
"""

_EXECUTOR_COMPATIBILITY_SCRIPT = r"""
<script src="/ui/runtime/executorCompatibility.js?v=5"></script>
<script>
(() => {
  const icons = [
  ...
  // dashboard.js re-renders the Start/Stop button after every status update.
  // Re-apply the requested artwork after every status render.
  ...
  // Replace the fixed Size Grid dropdown with a free-form grid picker.
  ...
  initGridPicker();
  setInterval(initGridPicker, 500);
  ...
  // Keep the three window arrangement controls at the top of the card.
  ...
  // Rename the navigation/page labels without changing the stable view id.
  ...
  // Use the supplied artwork for Search, Running and the Cronus logo.
  ...
  // Close only the selected accounts' Roblox clients; Executor processes stay open.
  ...
  // Display the latest API version instead of the locally installed version.
  ...
  setInterval(refreshCurrentVersion, 5000);
  ...
  // Limiter and Graphics share the same page header.  Keep one action pair
  // visible so editing cards from both sections cannot render duplicate
  // Reset/Save controls.  The hidden Graphics buttons remain the actual
  // handlers for their section and are invoked after the aggregate action.
  ...
  syncLimiterActions();
  setInterval(syncLimiterActions, 100);
})();
</script>
"""
...
def _load_html_ui() -> str:
    with open(resource_path("ui", "index.html"), "r", encoding="utf-8") as f:
        html = f.read()
    if "</body>" in html:
        return html.replace("</body>", _EXPLOITSTRAP_PICKER_SCRIPT + _EXECUTOR_COMPATIBILITY_SCRIPT + _UPDATE_BANNER_SCRIPT + "</body>", 1)
    return html + _EXPLOITSTRAP_PICKER_SCRIPT + _EXECUTOR_COMPATIBILITY_SCRIPT + _UPDATE_BANNER_SCRIPT
