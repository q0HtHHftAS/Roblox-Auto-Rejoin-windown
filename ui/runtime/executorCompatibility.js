(function () {
  ...
  function renderStatus(status) {
    ...
    if (state) state.textContent = status.state === "compatible" ? "Compatible" : status.state === "incompatible" ? "Not compatible" : status.state === "api_error" ? "WEAO unavailable" : status.state === "unknown" ? "Unknown" : "Disabled";
    if (status.error) show(`WEAO: ${status.error}`, true);
  }
  ...
  async function load() {
    try {
      ...
      if (selected && !available) {
        ...
        show(`Executor ${selected} was not found in WEAO and has been cleared`, true);
      }
  ...
  function bindCard() {
    ...
    $("executor-check-now")?.addEventListener("click", async () => { try { renderStatus(await api("/troubleshoot/executor/check", "POST", {})); show("WEAO check complete"); } catch (error) { show(error.message, true); } });
  }
