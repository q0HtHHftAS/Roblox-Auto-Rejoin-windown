/* Compact self-update button (classic script, no build step).
 * The service auto-downloads in the background. This only shows a small
 * button in the left sidebar when an update needs attention:
 * downloading (progress, disabled) or downloaded (press to restart).
 * Source runs (not compiled) never show anything.
 */
(function () {
  "use strict";
  ...
  function confirmInstall(version) {
    var back = document.createElement("div");
    back.className = "cu-overlay";
    back.id = "cu-overlay";
    var box = document.createElement("div");
    box.className = "cu-modal";
    box.innerHTML = "<h3>Install update v" + esc(version) + "?</h3>"
      + "<p>App will exit now and relaunch on the new version.</p>"
      + '<div class="cu-modal-actions"><button class="cu-btn" id="cu-cancel">Cancel</button>'
  ...
  function render(snap) {
    if (!snap || !snap.compiled || !snap.update_available) { removeButton(); return; }
    var st = snap.state || "idle";
    if (st === "downloaded") {
      var b = ensureButton();
      b.disabled = false;
      b.innerHTML = "<span>↻ Update v" + esc(snap.latest_version || "") + "</span>";
      b.title = "Install update and restart";
      b.onclick = function () {
        if (snap.farm_running) { toast("Stop Auto Rejoin first, then install.", true); return; }
        if (!snap.can_install) { toast("Update is not ready yet.", true); return; }
        confirmInstall(snap.latest_version || "");
      };
    } else if (st === "downloading" || st === "verifying" || st === "available" || st === "checking") {
      var p = ensureButton();
      p.disabled = true;
      var pct = Math.max(0, Math.min(100, Number(snap.progress_percent) || 0));
      p.innerHTML = "<span>↓ " + (st === "available" || st === "checking" ? "preparing…" : pct + "%") + "</span>";
  ...
  function refresh() {
    req("/update/status").then(function (snap) {
      render(snap);
      var busy = snap && (snap.state === "downloading" || snap.state === "verifying" || snap.state === "checking");
